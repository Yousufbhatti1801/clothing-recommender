...
class SearchService:
    ...
    async def search(
        ...
        Args:
            ...
            max_price:  When set, adds a Pinecone server-side filter so only
                        products with ``price <= max_price`` are returned.
                        This eliminates over-budget results before they ever
    ...
    async def search_many(
        ...
        for embedding, category in pairs:
            matches = await self.search(embedding, category, top_k, max_price)
